...
make_roedeer_dataset <- function(buffer_for_cs, buffer_for_telem, thinning, ratio_random_cs){
  ...
  telem_oslo <- read_sf('analysis/real_analysis/dataset/roedeer/Telem_RoeDeer_Oslo.shp') %>%
  ...
  oste_data_select <- readxl::read_excel(path = "analysis/real_analysis/dataset/roedeer/VHF_data.xlsx", sheet = 1) %>% 
    drop_na(WGS84_X2) %>% 
  ...
  akerhus_data_select <- readxl::read_excel(path = "analysis/real_analysis/dataset/roedeer/VHF_data.xlsx", sheet = 2)  %>% 
    drop_na(WGS84X) %>% 
  ...
  bbox <- read_sf("analysis/real_analysis/dataset/study_area/study_area.shp") %>% 
    st_transform(crs = 25833)
  ...
  # Citizen science
  CS <- openxlsx::read.xlsx('analysis/real_analysis/dataset/cs/cs_mammals.xlsx', sheet = 1)
