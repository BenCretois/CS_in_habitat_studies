...
get_covariate <- function(species, data, list_raster, n){
  ...
  if(species == 'wreindeer'){
    ...
    for(i in 1:length(list_raster)){
      r <- read_stars(paste0("analysis/real_analysis/covariates_reindeer/", list_raster[[i]]))
      ...
      for(i in 1:length(list_raster)){
        r <- read_stars(paste0("analysis/real_analysis/covariates_moose_roe/", list_raster[[i]]))
        data[ , i+n] <- raster_extract(r, data)
        rm(r)
      }
    }
