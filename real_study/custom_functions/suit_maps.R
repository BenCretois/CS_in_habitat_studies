...
# Plot the maps
a <- ggplot(intersect_data) + geom_sf(aes(fill = pred_telem), color = NA) + scale_fill_viridis(option = 'magma') + theme_minimal()
b <- ggplot(intersect_data) + geom_sf(aes(fill = pred_naive), color = NA) + scale_fill_viridis(option = 'magma') + theme_minimal()
c <- ggplot(intersect_data) + geom_sf(aes(fill = pred_corrected), color = NA) + scale_fill_viridis(option = 'magma') + theme_minimal()
d <- ggplot(intersect_data) + geom_sf(aes(fill = pred_sp1), color = NA) + scale_fill_viridis(option = 'magma') + theme_minimal()
e <- ggplot(intersect_data) + geom_sf(aes(fill = pred_sp2), color = NA) + scale_fill_viridis(option = 'magma') + theme_minimal()
